...
- **TDD is reinforced as the default.** Already true in v1, explicit in v2.1: every task ships with a `verify_test.go`; the tracker's `verify` command *is* the grade.
...
**What v2 does.** Four-level mastery per task — `unseen → learning → proficient → automatic` — advanced only by passing a test the tracker runs (`learning`), re-passing the test after a delay (`proficient`), and beating the drill time target (`automatic`). Checkboxes are gone.

---

## 2. No explicit prerequisite graph

**Gap.** v1 orders tasks sequentially but never says *why*. 1.4 (pointers/slices/maps) is listed after 1.3 (errors/testing) even though it only needs 1.1. Worse, nothing tells the tracker which earlier task to re-surface when a later one fails.
...
**What v2 does.** Every task frontmatter carries `prereqs: [...]` and `remediation: [...]`. The tracker refuses to let you start a task whose prereqs aren't `learning` yet, and when a verify fails twice it suggests the remediation target. See `internal/graph/dag.go`.
...
- Each task has `review_intervals_days: [3, 7, 21, 60]`. The tracker resurfaces every task at those offsets via the `review` queue. A missed review *demotes* mastery.
...
**What v2 does.** `tracker place phase-N` runs a short timed challenge at phase entry. Passing marks earlier tasks as placement-skipped in `progress.json.placement`. Phase-1–3 placements are concrete coding exercises; Phase 4 is a concept MCQ; Phase 5 has none (Maelstrom is the judge).
...
- The task detail page in the tracker UI shows the reflection log inline with attempt history.

---

## 7. Phase 5 is too coarse-grained

**Gap.** v1 bundles Broadcast 3a–3e into one bullet list, same for Kafka 5a–5c and Txn 6a–6c. The tracker can't tell whether you're at 3b or 3d — the smallest progress unit is "the whole challenge."
...
**What v2 does.** The tracker: two consecutive `verify` failures on task N trigger a prompt to run `tracker verify <remediation-id>` first. The remediation ID comes from the task's frontmatter.
...
**What v2 does.** 1.6 remains a task (property tests, reflection, SVG clock) but is explicitly `interleave_with: [1.4, 1.5]` — you do it alongside, not after. Mapped LGwT chapters now live on each task's frontmatter (`lgwt_chapters:`) so the tracker can link directly.
...
**What v2 does.** Every task has a concrete verify — unit tests in `exercises/phase-*/<task-id>/verify_test.go` for Phases 1–4, a Maelstrom invocation for Phase 5. The tracker CLI runs them and writes the result to `progress.json`. You can't mark yourself complete; the test marks you.
