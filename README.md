...
It ships with `go-dojo`, a local CLI + web dashboard (Go, stdlib-only) that enforces **Math-Academy-Way** learning: mastery levels instead of checkboxes, spaced retrieval, prerequisite DAG, timed drills for automaticity, phase-entry placement quizzes.
...
```sh
./setup.sh                             # check go, build ./go-dojo, generate aliases.sh
source ./aliases.sh                    # enables the 'go-dojo' shell function
go-dojo serve                         # dashboard on http://localhost:8080
go-dojo placement phase-0             # diagnostic quiz — skip what you already know
go-dojo verify 1.1-hello-world        # run the first exercise's tests
```

To make `go-dojo` available in every new shell, add to `~/.zshrc` (or `~/.bashrc`):

```sh
source /path/to/learning-plan/aliases.sh
```

Open `exercises/phase-1/1.1-hello-world/` — fill in `echo.go`, re-run `go-dojo verify`. When it goes green, your mastery on `1.1-hello-world` becomes `learning` and a review is scheduled 3 days out.

> Without the function sourced, every command still works in long form: `go run ./cmd/go-dojo serve` or `./go-dojo serve` after `./setup.sh`.
...
- **Prereq DAG.** Every task declares its prerequisites; `go-dojo` refuses to surface a task whose prereqs aren't at least `learning`. Failed verifies route you to the remediation target.
...
- **Diagnostic placement** at phase entry lets you skip what you already own: `go-dojo placement phase-0`.
...
├── go.mod
├── cmd/go-dojo/             # the CLI binary (serve/verify/drill/review/placement/validate)
...
With the `go-dojo` function sourced (see Quick start):

```sh
go-dojo serve                   # dashboard, graph, review queue, drills
go-dojo verify    <task-id>     # run that task's tests, update mastery
go-dojo drill     <drill-id>    # timed deliberate-practice exercise
go-dojo review                  # what's due for retrieval today
go-dojo placement <phase-id>    # diagnostic quiz (phase-0 … phase-4)
go-dojo validate  [path]        # lint a curriculum / skill-tree file
```

Long forms (no function needed): `go run ./cmd/go-dojo <subcommand>` or `./go-dojo <subcommand>` after `./setup.sh`.
